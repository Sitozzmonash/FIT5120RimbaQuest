...
def migrate_schema() -> None:
    ...
    try:
        with sqlite3.connect(database_path()) as connection:
            ...
            # Keep one card entry per child and species, including on existing persistent disks.
            ...
            # A badge is awarded once per child. Existing rows are retained by lowest id.
            ...
            # Registration checks usernames and emails in the API; these indexes make the
            # same rule reliable when requests arrive concurrently.
            connection.execute("""CREATE UNIQUE INDEX IF NOT EXISTS
