...
def list_child_facing_fun_facts(species_id: str):
    ...
    with engine.connect() as connection:
        species = connection.execute(
            text("""SELECT common_name, scientific_name, distinctive_features, habitat,
                           diet, conservation_status, fun_fact, source_name, source_ref
        ...
        facts = rows(connection.execute(
            text("""SELECT display_order, fact_text, source_name, source_url
    ...
    for fallback in fallback_candidates:
        ...
        if text_value and text_value.casefold() not in existing_text and len(facts) < 10:
            facts.append({
                "display_order": 100 + len(facts),
                "fact_text": text_value,
                "source_name": species["source_name"] or "RimbaQuest wildlife catalogue",
                "source_url": species["source_ref"],
            })
