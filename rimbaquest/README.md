...
The Wildlife Card Battle screen supports AI Bot practice and invited friend matches. The backend shows a random habitat before card selection, checks the chosen card's habitat and two-battle rest status (the least-rested card stays playable when every card is resting), and resolves every action. A friend can join with the displayed code; the screen refreshes the match every second while the backend enforces each 30-second turn. Quiz progress unlocks the three abilities, which cost 1, 2, and 4 Energy. The friend leaderboard is separate from Explorer XP.
